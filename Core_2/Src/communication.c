...
//	int len = snprintf(tx_msg, sizeof(tx_msg), "%s", m);
//
//	HAL_UART_Transmit(&hlpuart1, (uint8_t*) tx_msg, len,100);
}

void Communication_GetActuationData(Global_State_t *g, Decision_t *d,
		uint8_t *disc) {
	*g = global_state;
	*d = decision_b2;
	*disc = is_discordant;
...
void Communication_Init(void) {
	...
	Controller_Bus c = { .battery = 155, .circle = 0, .square = 0, .cross = 0,
			.triangle = 0, .ry = 345, .l1 = 0, .l2 = 0, .r1 = 0, .r2 = 0 };

	partial_state_b2.angolo = 82;
	partial_state_b2.controller = c;
	partial_state_b2.controller_status = 1;
	partial_state_b2.dC = 400;
	partial_state_b2.dL = 400;
	partial_state_b2.dR = 400;
	partial_state_b2.degradato = 0;
	partial_state_b2.emergenza = 0;
	partial_state_b2.mpu_status = 1;
	partial_state_b2.sonar_status = 1;

	HAL_GPIO_WritePin(BOARD_ALIVE_B2_GPIO_Port, BOARD_ALIVE_B2_Pin,
			GPIO_PIN_SET);
...
void Communication_Step(void) {
	...
	do {
		...
		case COMM_STATE_START:
			...
			if (HAL_GPIO_ReadPin(BOARD_ALIVE_B1_GPIO_Port, BOARD_ALIVE_B1_Pin)
					== GPIO_PIN_RESET) {
				currentState = COMM_STATE_SAFE_MODE;
				printDebug("START\tBOARD_ALIVE_B1 SPENTO\n\r");
			} else {
			...
			if (currentState != COMM_STATE_RECEIVE_PACKET_B2) {
				currentState = COMM_STATE_SAFE_MODE;
				printDebug("START\tTIMEOUT RTS_B1\n\r");
			}
		...
		case COMM_STATE_READ_PACKET:
			printDebug("READ_PACKET\n\r");
			do {
				if (HAL_UART_Receive(&huart1, rx_partial_buffer, PACKET_B1_SIZE,
				UART_TIMEOUT_PARTIAL_MS) == HAL_OK) {
					int16_t temp_seq;
					if (ReadUart_packet_B1_Local(rx_partial_buffer,
					PACKET_B1_SIZE, &partial_state_b1, &temp_seq)) {
						success_read = 1;
						seq_num = temp_seq;
						printDebug("READ_PACKET\tDATI RICEVUTI\n\r");
					} else {
						success_read = 0;
		...
		case COMM_STATE_SERIALIZE_PACKET_B2:
			printDebug("SERIALIZE_PACKET_B2\n\r");
			...
			if (currentState != COMM_STATE_SEND_PACKET_B2) {
				currentState = COMM_STATE_SAFE_MODE;
				printDebug("TIMEOUT_CTS_B1\n\r");
		...
		case COMM_STATE_SEND_PACKET_B2:
			printDebug("SEND_PACKET_B2\n\r");


			do {
				if (HAL_UART_Transmit(&huart1, buffer_b2, PACKET_B2_SIZE,
				UART_TIMEOUT_PARTIAL_MS) == HAL_OK) {
					success_send = 1;
				} else {
					success_send = 0;
		...
		case COMM_STATE_PACKET_SENT:
			...
			if (currentState != COMM_STATE_GLOBAL_STATE_RECONSTRUCTION) {
				printDebug("TIMEOUT_CTS_B1\n\r");
		...
		case COMM_STATE_GLOBAL_STATE_RECONSTRUCTION:
			printDebug("GLOBAL_STATE_RECONSTRUCTION\n\r");
		...
		case COMM_STATE_TAKE_DECISION_B2:
			...
			if (currentState != COMM_STATE_READ_DECISION_B1) {
				printDebug("TIMEOUT_RTS_B1\n\r");
		...
		case COMM_STATE_READ_DECISION_B1:
			printDebug("READ_DECISION_B1\n\r");
			HAL_GPIO_WritePin(CTS_B2_GPIO_Port, CTS_B2_Pin, GPIO_PIN_SET);
			do {
				if (HAL_UART_Receive(&huart1, rx_decision_buffer,
				DECISION_PACKET_SIZE, UART_TIMEOUT_DECISION_MS) == HAL_OK) {
					int16_t temp_seq;
					if (ReadUart_Decision_Local(rx_decision_buffer,
					DECISION_PACKET_SIZE, &decision_b1, &temp_seq)) {
						success_read = 1;
						seq_num = temp_seq;
					} else {
						success_read = 0;
		...
		case COMM_STATE_SERIALIZE_DECISION_B2:
			printDebug("SERIALIZE_DECISION_B2\n\r");
			HAL_GPIO_WritePin(RTS_B2_GPIO_Port, RTS_B2_Pin, GPIO_PIN_SET);
			...
			if (currentState != COMM_STATE_SEND_DECISION_B2) {
				printDebug("TIMEOUT_CTS_B1\n\r");
		...
		case COMM_STATE_SEND_DECISION_B2:
			printDebug("SEND_DECISION_B2\n\r");

			do {
				if (HAL_UART_Transmit(&huart1, buffer_decision,
				DECISION_PACKET_SIZE, UART_TIMEOUT_DECISION_MS) == HAL_OK) {
					success_send = 1;
				} else {
					success_send = 0;
		...
		case COMM_STATE_DECISION_SENT:
			...
			if (currentState != COMM_STATE_ACTUATE_DECISION) {
				printDebug("TIMEOUT_CTS_B1\n\r");
			...
			if (HAL_UART_GetState(&hlpuart1) == HAL_UART_STATE_READY) {
				...
				int len =
						snprintf(tx_msg, sizeof(tx_msg),
								"B1: V[%.2f, %.2f, %.2f, %.2f] Bat:%u Temp:%u Photo:%ld Enc:%u B_Stat:%d T_Stat:%u Em:%u Deg:%u | "
								...
								global_state.b2.dL, global_state.b2.dC,
								global_state.b2.dR, global_state.b2.angolo,
				...
				HAL_UART_Transmit(&hlpuart1, (uint8_t*) tx_msg, len, 20);
			}*/
