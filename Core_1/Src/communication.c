...
void Communication_Init(void) {
	...
	HAL_GPIO_WritePin(BOARD_ALIVE_B1_GPIO_Port, BOARD_ALIVE_B1_Pin,
			GPIO_PIN_SET);
...
void Communication_Step(void) {
	...
	do {
		...
		case COMM_STATE_START:
			...
			if (HAL_GPIO_ReadPin(BOARD_ALIVE_B2_GPIO_Port, BOARD_ALIVE_B2_Pin)
					== GPIO_PIN_RESET) {
				currentState = COMM_STATE_SAFE_MODE;
				printDebug("START\tBOARD_ALIVE_B2 SPENTO\n\r");
			} else {
		...
		case COMM_STATE_SERIALIZE_SENSOR_DATA:
			printDebug("SERIALIZE_SENSOR_DATA\n\r");
			//if (osMutexAcquire(partialStateMutexHandle, 0) == osOK) {
			serializePartialStateB1(&partial_state_b1, (uint8_t) seq_num,
					tx_buffer);
			//osMutexRelease(partialStateMutexHandle);
			//}
			tick_start = HAL_GetTick();
			while (HAL_GetTick() - tick_start < TIMEOUT_MS) {
				if (HAL_GPIO_ReadPin(CTS_B2_GPIO_Port, CTS_B2_Pin)
						== GPIO_PIN_SET) {
			...
			if (currentState != COMM_STATE_SERIALIZE_SEND_B1) {
				currentState = COMM_STATE_SAFE_MODE;
				printDebug("START\tTIMEOUT CTS_B2\n\r");

			}
			break;

		case COMM_STATE_SERIALIZE_SEND_B1:
			printDebug("SERIALIZE_SEND_B1\n\r");

			do {
				if (HAL_UART_Transmit(&huart1, tx_buffer, PACKET_B1_SIZE,
		...
		case COMM_STATE_PACKET_SENT:
			...
			tick_start = HAL_GetTick();
			while (HAL_GetTick() - tick_start < TIMEOUT_MS) {
				if (HAL_GPIO_ReadPin(RTS_B2_GPIO_Port, RTS_B2_Pin)
						== GPIO_PIN_SET) {
					currentState = COMM_STATE_RECEIVE_PACKET;
					break;
				}
			}
			if (currentState != COMM_STATE_RECEIVE_PACKET) {
				printDebug("TIMEOUT_RTS_B2\n\r");
				currentState = COMM_STATE_SAFE_MODE;
			}
			break;

		case COMM_STATE_RECEIVE_PACKET:
			printDebug("RECEIVE_PACKET\n\r");
		...
		case COMM_STATE_READ_PACKET:
			...
			do {
				if (HAL_UART_Receive(&huart1, packet_rx_buffer, PACKET_B2_SIZE,
				UART_TIMEOUT_PARTIAL_MS) == HAL_OK) {
					int16_t temp_seq;
					if (ReadUart_packet_B2(packet_rx_buffer, PACKET_B2_SIZE,
							&partial_state_b2, &temp_seq)) {
						success_read = 1;
						seq_num = temp_seq;
					} else {
						success_read = 0;
		...
		case COMM_STATE_SERIALIZE_DECISION_B1:
			printDebug("SERIALIZE_DECISION_B1\n\r");

			HAL_GPIO_WritePin(RTS_B1_GPIO_Port, RTS_B1_Pin, GPIO_PIN_SET);
			serializeDecisionB1(&decision_b1, (uint8_t) seq_num,
					tx_buffer_decision);
			...
			if (currentState != COMM_STATE_DECISION_SEND) {
				printDebug("TIMEOUT_CTS_B2\n\r");
		...
		case COMM_STATE_DECISION_SEND:
			printDebug("DECISION_SEND\n\r");

			do {
				if (HAL_UART_Transmit(&huart1, tx_buffer_decision,
		...
		case COMM_STATE_DECISION_SENT:
			...
			if (currentState != COMM_STATE_READ_DECISION_B2) {
				printDebug("TIMEOUT_RTS_B2\n\r");
		...
		case COMM_STATE_READ_DECISION_B2:
			printDebug("READ_DECISION_B2\n\r");

			HAL_GPIO_WritePin(CTS_B1_GPIO_Port, CTS_B1_Pin, GPIO_PIN_SET);
			do {
				if (HAL_UART_Receive(&huart1, buffer_decision,
				DECISION_PACKET_SIZE, UART_TIMEOUT_DECISION_MS) == HAL_OK) {
					int16_t temp_seq;
					if (ReadUart_decision_B2(buffer_decision,
					DECISION_PACKET_SIZE, &decision_b2, &temp_seq)) {
						success_read = 1;
						seq_num = temp_seq;
					} else {
						success_read = 0;
		...
		case COMM_STATE_ACTUATE_DECISION:
			...
			/*if (HAL_UART_GetState(&hlpuart1) == HAL_UART_STATE_READY) {
				...
				HAL_UART_Transmit(&hlpuart1, (uint8_t*) tx_msg, len, 50);
			}*/
			currentState = COMM_STATE_START;
			processing_complete = 1;
			break;
